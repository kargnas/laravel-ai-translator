...
class TranslateCrowdinParallel extends TranslateCrowdin
{
        ...
        try {
            ...
            while (!empty($queue) || !empty($running)) {
                while (count($running) < $maxProcesses && !empty($queue)) {
                    ...
                    $process = new Process($this->buildLanguageCommand($lang, $token, $organization), base_path());
    ...
    private function buildLanguageCommand(string $language, string $token, ?string $organization): array
    {
        $cmd = [
            'php',
            'artisan',
        ...
        if ($this->option('project')) {
            $cmd[] = '--project=' . $this->option('project');
        }
        if ($this->option('source-language')) {
            $cmd[] = '--source-language=' . $this->option('source-language');
        }
